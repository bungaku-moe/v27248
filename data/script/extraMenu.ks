[if exp="typeof(global.exmenu_object) == 'undefined'"]
[iscript]


class ExMenuLayer extends ExButtonLayer
{
	var menuenable = true;
	var onenablecheck = 'true';
	
	function ExMenuLayer(window, parent)
	{
...
class ExMenuPlugin extends ExPlugin
{
	var triggerBtn = %[];
	var fullAreaBtn = %[];
	var show = false;
	...
	function ExMenuPlugin()
	{
		super.ExPlugin('ExMenuPlugin');
	}
	...
	function createObject(elm)
	{
		if (!ready) {
			ready = true;
			triggerBtn.back = createTriggerBtn(kag, kag.back.base, 0, 0, kag.innerWidth, 20);
			triggerBtn.fore = createTriggerBtn(kag, kag.fore.base, 0, 0, kag.innerWidth, 20);
	...
	{
		var obj = new ExMenuLayer(window, parent);
		with (obj) {
			.loadImages(elm.file);
			.setPos(elm.x, elm.y);
			.absolute = 2000000-3;
		...
		obj.onMouseEnter = function()
		{
			global.exmenu_object.showChild(name);
			(global.ExButtonLayer.onMouseEnter incontextof this)(...);
		} incontextof obj;
		obj.onClick = function()
		{
			global.exmenu_object.hideMenu();
			(global.ExMenuLayer.onClick incontextof this)(...);
		} incontextof obj;
		return obj;
	}
...
kag.addPlugin(global.exmenu_object = new ExMenuPlugin(kag));
...
;------------------------------------------------------------------------------
;	メニュー
;		[exmenu name="" parent="" x="" y="" file="" onclick="" onenter="" onleave=""]
;		name		識別子
;		parent		親の識別子
;		x			x座標
;		y			y座標
...
;------------------------------------------------------------------------------
[macro name="exmenu"]
	[eval exp="exmenu_object.createObject(mp)"]
[endmacro]
...
;------------------------------------------------------------------------------
[macro name="exmenuopt"]
	[eval exp="exmenu_object.setOptions(mp)"]
[endmacro]
