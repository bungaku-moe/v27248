...
class ZoomPlugin extends KAGPlugin
{
	...
	function startlayerZoom(sourcelayer, layer, mode, basestorage, sl, st, sw, sh, dl, dt, dw, dh, time, accel)
	{
		// storage : 表示画像
		// layer : 対象レイヤ
		// mode : 前景レイヤの透過モード
		...
		{
			tempLayer = new Layer(window, base);
			tempLayer = kag.back.layers[+sourcelayer];
//			tempLayer.loadImages(storage);
//			tempLayer.copyRect(0,0,kag.back.layers[+sourcelayer],0,0,kag.back.layers[+sourcelayer].clipHeight,kag.back.layers[+sourcelayer].clipWidth);
		}
...
@macro name="layerzoom"
@eval exp="zoom_object.startlayerZoom(mp.sourcelayer, mp.layer, mp.mode, void, +mp.sl, +mp.st, +mp.sw, +mp.sh, +mp.dl, +mp.dt, +mp.dw, +mp.dh, +mp.time, +mp.accel)"
@endmacro
